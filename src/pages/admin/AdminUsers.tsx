...
export default function AdminUsers() {
  ...
  async function changePassword(e: React.FormEvent) {
    ...
    try {
      ...
      flash('パスワードを変更しました');
      setPwUserId(null); setNewPassword('');
    } catch (err: any) {
      flash(err.response?.data?.message || 'エラーが発生しました', true);
    }
  }
        ...
        <Modal title={editId ? 'ユーザー編集' : 'ユーザー追加'} onClose={() => setShowForm(false)}>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <select value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} className="input-luxury">
              ...
              <tr key={u.id} className={`border-b border-white/5 glass-hover transition ${!u.active ? 'opacity-40' : ''} ${i === users.length - 1 ? 'border-b-0' : ''}`}>
                ...
                <td className="px-5 py-4">
                  <div className="flex gap-3 text-xs">
                    ...
                    <button onClick={() => toggleActive(u)} className="text-white/40 hover:text-white/70 transition">{u.active ? '無効化' : '有効化'}</button>
                    <button onClick={() => setPwUserId(u.id)} className="text-white/40 hover:text-white/70 transition">PW変更</button>
                    <button onClick={() => deleteUser(u.id)} className="text-red-500/60 hover:text-red-400 transition">削除</button>
                  </div>
                </td>
              </tr>
            ))}
